CREATE  TABLE IF NOT EXISTS `sotm_category_x_tag_x_page` (
...
PRIMARY KEY (`category_id`, `tag_id`, `page_num`),
UNIQUE INDEX `uk_uri` (`uri` ASC),
INDEX `fk_sotm_category_x_tag__tag_id` (`tag_id` ASC),
INDEX `indx_deleted` (`is_deleted` ASC, `date_deleted` ASC),
INDEX `indx_edited` (`is_edited` ASC, `date_modified` ASC),
CONSTRAINT `fk_sotm_category_x_tag__category_tag`
FOREIGN KEY (`category_id`, `tag_id` )
REFERENCES `sotm_category_x_tag` (`category_id`, `tag_id` )
ON DELETE CASCADE)
ENGINE = InnoDB;