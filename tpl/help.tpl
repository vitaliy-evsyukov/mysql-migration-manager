...
Список доступных команд:
    ...
    verify                          Проверить, есть ли в вашей БД внесенные вами локально изменения. На экран будут выведены изменения, если таковые присутствуют
...
Опции конфигурации могут задаваться двумя способами:
    ...
    --tmp_add_suffix                Если значение кастуется к логической истине, то при имена временных БД заканчиваются на метку времени UNIX. При  апгрейде суффиксом имени будет "full_temp_db"
    --tmp_db_name                   Имя временной БД, в которой будет производиться поиск связей и в которой будут разворачиваться миграции и схема для сравнения. Если tmp_add_suffix не указан или может быть приведен к логическому значению ЛОЖЬ, то к имени не будет ничего добавлено
...
Список доступных аргументов команд:
    ...
    --verify                        Проверить, не содержит ли ваша БД внесенных вами локально изменений перед совершением миграции. Если такие есть, миграция совершена не будет
